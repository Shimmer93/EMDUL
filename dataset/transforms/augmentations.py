...
class AddPointsAroundJoint():
    ...
    def __call__(self, sample):
        ...
        num2add = np.random.randint(1, self.max_num2add + 1)
        ...
        for i in range(len(sample['keypoints'])):
            ...
            for idx in idxs2add:
                ...
                add_points = add_point[np.newaxis, :].repeat(self.num_added, axis=0) + np.random.normal(0, self.add_std, (self.num_added, pc.shape[-1]))
    ...
    def __call__(self, sample):
        for i in range(len(sample['point_clouds'])):
            ...
            if self.outlier_type == 'statistical':
                ...
                dist_threshold = np.mean(mean_dist) + self.std_multiplier * np.std(mean_dist)
                inliers = np.where(mean_dist < dist_threshold)
...
class MultiFrameAggregate():
    ...
    def __call__(self, sample):
        ...
        if self.num_frames <= total_frames:
            start = self.offset
            stop = total_frames - self.offset
            sample['point_clouds'] = [
                np.concatenate(sample['point_clouds'][i - self.offset:i + self.offset + 1])
                for i in range(start, stop)
            ]
            if 'keypoints' in sample:
                if self.offset == 0:
                    sample['keypoints'] = sample['keypoints']
                else:
