...
def rodrigues_2_rot_mat(rvecs):
    ...
    is_zero = torch.eq(torch.squeeze(thetas), torch.tensor(0.0, device=device))
    u = r_vecs / thetas

    # Each K is the cross product matrix of unit axis vectors
    # pyformat: disable
    zero = torch.zeros([total_size], device=device)
    ...
    identity_mat = torch.eye(3, device=device).repeat(total_size, 1, 1)
