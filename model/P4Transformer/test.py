...
class P4Transformer(nn.Module):
    ...
    def forward(self, xyzs, rgbs):
        ...
        print(new_xyzsd2.size(), xyzs.size(), new_featuresd2.size(), rgbs.size())
        new_xyzsd1, new_featuresd1 = self.deconv1(new_xyzsd2, xyzs, new_featuresd2, rgbs)

        print(new_featuresd1.size())
...
if __name__ == '__main__':
    model = P4Transformer().cuda()
    xyzs = torch.randn(2, 16, 1024, 3).cuda()
    rgbs = torch.randn(2, 16, 3, 1024).cuda()
    out = model(xyzs, rgbs)
    print(out.size())
